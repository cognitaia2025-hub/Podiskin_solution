...
async def get_pacientes(
    ...
    orden: str = Query("nombre", description="Field to order by (nombre, fecha_registro, fecha_nacimiento)"),
    ...
    try:
        ...
        if paciente.curp:
            exists = await conn.fetchval(
                "SELECT EXISTS(SELECT 1 FROM pacientes WHERE curp = $1)",
                paciente.curp
            )
            if exists:
                raise HTTPException(
...
@router.post("/{paciente_id}/antecedentes", response_model=AntecedenteResponse, status_code=status.HTTP_201_CREATED)
async def create_antecedente(
    ...
    **Raises:**
    ...
    """
    try:
        return await AntecedentesService.create_antecedente(conn, paciente_id, antecedente)
