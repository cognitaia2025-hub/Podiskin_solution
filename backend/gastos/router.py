...
from citas.database import _get_connection as get_db_connection_citas  # Para transacciones
...
class GastoCreate(BaseModel):
    categoria: str = Field(..., description="Categoría del gasto (nuevo formato con enum)")
...
class GastoConInventarioRequest(BaseModel):
    ...
    registrado_por: Optional[int] = None
    notas: Optional[str] = None
    productos: List[ProductoInventario] = Field(..., min_items=1, description="Lista de productos a actualizar en inventario")


@router.get("/")
async def listar_gastos(
    categoria: Optional[str] = Query(None, description="Categoría del gasto (nuevo o legacy)"),
...
async def crear_gasto_con_inventario(request: GastoConInventarioRequest):
    ...
    """
    # Validaciones
    if request.categoria not in CATEGORIAS and request.categoria not in CATEGORIAS_LEGACY:
        raise HTTPException(
            400, 
            f"Categoría inválida. Use: {CATEGORIAS} o {CATEGORIAS_LEGACY}"
        )
    ...
    try:
        ...
        insert_gasto_query = """
            ...
            RETURNING id
        """
        cur.execute(insert_gasto_query, (
            request.categoria,
            request.concepto,
            request.monto,
